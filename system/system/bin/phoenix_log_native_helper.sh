...
ERROR_FUNC_MAP_KEY=("ERROR_HANG_OPPO"
                    ...
                    "ERROR_MTK_REBOOT_FROM_WDT_UNKNOWN_2SEC_SUCCESS"
                    )
...
ERROR_FUNC_MAP_VAL=("collect_hang_oppo_log"
                    ...
                    "collect_mtk_reboot_from_wdt_unknow_2sec_log"
                    )
...
#Jiang.Li@PSW.TECH.Stability, 2020/01/21, Add expdb log when device boot from WDT/BR_UNKNOW/BR_2SEC_REBOOT
function collect_mtk_reboot_from_wdt_unknow_2sec_log()
{
    tmp_dir=$1
	if [ "x${HARDWARE}" == "xqcom" ]; then
		echo "QCOM hasn't expdb,return"
		return 0
	fi

#cp boot_log to tmp_dir
    ...
    then
        cp -a ${PHX_LOG_DIR}/${latest_file_boot} ${tmp_dir}/${latest_file_boot}
		rm ${PHX_LOG_DIR}/${latest_file_boot}
    fi

#cp expdp to tmp_dir
    cache_remainsize=$(df -k | grep "cache" | sed 's/[ ][ ]*/,/g' | cut -d "," -f4)
    echo "cache_remainsize=${cache_remainsize}"
    #Save expdb to tmp_dir when cache size more then 40MB
    if [ "cache_remainsize" -gt "40960" ] ;
    then
        dd if=/dev/block/by-name/expdb of=${tmp_dir}/expdb
    fi
}
